      ...
      _conv(Convertion)
{
    ...
    if(_owner->tagPrefix != "" && _DBName != ""){
        ...
        if(_tag != nullptr){
            ...
            _logging (Prom::MessAlarm, "тэг в БД отсутствует", false);
        }
    }
    else {
        _owner->setAlarmTag("ИМЯ НЕ ЗАДАНО");
        _logging (Prom::MessAlarm, "пустое имя тэга", false);
    }
    _owner->addETag(this);
}
...
void ETag::_qualityChangedSlot()
{
    if(_tag->ReadQuality() == Prom::Good){
        ...
        if(! _imit){
            _alarm = true;
            emit s_alarm("потеря соединения");
        }
        else{
            _logging (Prom::MessVerbose, "соединение разорванно", false);
        }
    }
...
bool ETag::resetAlarm()
{
    //_alarmSetTime = false;
    //if(_alarm){
    if(_mayResetAlarm && connected()){
        _logging(Prom::MessVerbose, "авария сброшена", false);
        _alarm = false;
        emit s_alarmReseted();
        return true;
    }
    else if(_ignorAlarm) {
        _logging (Prom::MessAlarm, "авария сброшена по игнорированию", false);
        emit s_alarm("");
        //emit AlarmReseted();
        return true;
    }
    else {
        _logging(Prom::MessVerbose, "авария не сброшена", false);
        emit s_alarm("");
        return false;
    }
}
...
void ETag::writeIgnorAlarm(bool ignor)
{
    if(_ignorAlarm != ignor){
        ...
        if(_alarm && ! ignor)
            emit s_alarm(_name + " - восстановление аварии из-за отключения её игнорирования");
    }
}
...
void ETag::connectToGUI(QObject *guiItem, QObject *propWin)
{
    ...
    if(! this->isOk()){
        QMetaObject::invokeMethod(propWin, "addAlarm", Qt::DirectConnection, Q_RETURN_ARG(QVariant, ret),
                                  ...
                                  Q_ARG(QVariant, "Ошибка загрузки тэга - " + this->getName()),
    ...
    connect(this,SIGNAL(s_alarmReseted()            ), tmpSgSt, SLOT(resetAlarm()               ), Qt::QueuedConnection);
