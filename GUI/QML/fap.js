...
const manual = "yellow"
const middle = "lime"
const notConnected = "DarkSlateGray"
...
function changeColorG(node) {
    ...
    if (!node.linked) {
        ...
        if (node.manualOpenMove) {
            node.fillColor1 = manual
        }
        if (node.manualCloseMove) {
            node.fillColor2 = manual
        }
    } else {
...
function isInArea(p, size, point) {
    ...
    for (var i = 0; i < size; i++) {
        if ((p[i].Y < point.Y && p[j].Y >= point.Y || p[j].Y < point.Y && p[i].Y
             >= point.Y) && (p[i].X + (point.Y - p[i].Y) / (p[j].Y - p[i].Y)
                             * (p[j].X - p[i].X) < point.X))
            result = !result
        j = i
    }
