...
void Unit::connectToGUI(const QObject * GUI)
{
    ...
    connect(this, SIGNAL(s_connected()), propWin, SLOT(connected()), Qt::QueuedConnection);
    connect(this, SIGNAL(s_disconnected()), propWin, SLOT(disconnected()), Qt::QueuedConnection);
    connect(this, SIGNAL(SetInRoute(QVariant)), propWin, SLOT(setRoute(QVariant)), Qt::QueuedConnection);

    connect(this,    SIGNAL(s_alarm(QString)), propWin, SLOT(alarmSet()), Qt::QueuedConnection);
    connect(this,    SIGNAL(s_quitAlarm(QString)), propWin, SLOT(quitAlarm()   ), Qt::QueuedConnection);
    connect(this,    SIGNAL(s_alarmReseted()      ), propWin, SLOT(alarmReseted()), Qt::QueuedConnection);
